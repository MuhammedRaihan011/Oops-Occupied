...
<div style="margin: 50px; padding:20px;">
    <h3 style="margin:10px;">Vendor registration</h3>
...
if(isset($_REQUEST['btnSubmit']))
{
    ...
    {       
        $q="insert into tblvendor (vName,vAddress,vContact,vEmail,vFile,vIdType) values('$name','$address','$contact','$email','$file','$idtype')";
        $s= mysqli_query($conn, $q);
//        echo $q;
        if($s)  
        {
            $q="insert into tbllogin(uname,pwd,utype,status) values('$email','$pwd','vendor','0')";
            $s= mysqli_query($conn, $q);
            if($s)
            {
                ...
                echo '<script>location.href="vendorreg.php"</script>';
            }
            else
            {
